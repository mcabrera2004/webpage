        ...
        <nav className={`main-nav ${open ? 'active' : ''}`}>
          <ul style={{ display: 'flex', listStyle: 'none' }}>
            {['Inicio','Artículos','Libros','Estudios','Recursos','Acerca de'].map(item => (
              <li key={item} style={{ marginLeft: '1.5rem' }}>
                <a href={`/${item.toLowerCase().replace(/ /g,'')}`} style={{ color:'#fff', fontFamily:'var(--font-sans)', fontWeight:600, fontSize:'1rem', textTransform:'uppercase', letterSpacing:'0.5px' }}>{item}</a>
              </li>
            ))}
          </ul>
        </nav>
